...
const Contact = () => {
  ...
  const lineAnims = useMemo(
    () => headlineLines.map(() => ({ delay: 0.8 + Math.random() * 0.3, y: 6 + Math.random() * 10, rotate: (Math.random() - 0.5) * 3 })),
    [headlineLines]
  );
    ...
    <section id="contact" className="py-24">
      <div className="container max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          {/* Left: animated headline and 3D sphere */}
          <div className="order-1 md:order-1">
            <div className="text-xl md:text-2xl leading-relaxed text-primary/90 mb-8">
              {headlineLines.map((line, lineIdx) => (
                <motion.div
                  key={`line-${lineIdx}`}
                  initial={{ opacity: 0, y: lineAnims[lineIdx].y, rotate: lineAnims[lineIdx].rotate }}
                  whileInView={{ opacity: 1, y: 0, rotate: 0 }}
                  viewport={{ once: true, margin: '-40px' }}
                  transition={{ duration: 0.45, delay: lineAnims[lineIdx].delay, ease: 'easeOut' }}
                  className={lineIdx === 2 ? "text-4xl mt-4 text-center" : "mb-2"}
                >
                  {line}
                </motion.div>
              ))}
